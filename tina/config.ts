...
const abilityField = (name: string, label: string) => ({
  type: "object" as const,
  name,
  label,
  fields: [
    { type: "number" as const, name: "score", label: "Score" },
    { type: "string" as const, name: "mod", label: "Modifier" },
  ],
});
    ...
    collections: [
      {
        ...
        fields: [
          ...
          // ── Card 2: Combat, Magic & Forms ────────────────────────────────
          {
            ...
            list: true,
            ui: {
              itemProps: (item: { name?: string }) => ({ label: item?.name ?? "Attack" }),
            },
            fields: [
          ...
          },
          {
            ...
            list: true,
            ui: {
              itemProps: (item: { name?: string }) => ({ label: item?.name ?? "Feature" }),
            },
            fields: [
              { type: "string", name: "name", label: "Name" },
              { type: "string", name: "description", label: "Description", ui: { component: "textarea" } },
          ...
          // ── Card 4: Allies, Enemies & Gear ───────────────────────────────
          {
            ...
            list: true,
            ui: {
              itemProps: (item: { name?: string }) => ({ label: item?.name ?? "Entry" }),
            },
            fields: [
              { type: "string", name: "name", label: "Name" },
              { type: "string", name: "description", label: "Description", ui: { component: "textarea" } },
